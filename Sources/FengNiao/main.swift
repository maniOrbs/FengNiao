...
let projectPathOption = StringOption(
    ...
    helpMessage: "Root path of you Xcode project. Default is current folder.")
cli.addOption(projectPathOption)

let isForceOption = BoolOption(
    longFlag: "force", helpMessage: "Delete the found files without asking.")
cli.addOption(isForceOption)

let excludePathOption = MultiStringOption(
    shortFlag: "e", longFlag:"exclude",
    helpMessage: "Exclude paths form search.")
cli.addOption(excludePathOption)

let resourceExtOption = MultiStringOption(
    shortFlag: "r", longFlag: "resource-extensions",
    helpMessage: "Resource file extesions need to be searched. Default is 'imageset jpg png git pdf'")
cli.addOption(resourceExtOption)


let fileExtOption = MultiStringOption(
    shortFlag: "f", longFlag: "file-extensions",
    helpMessage: "In which types of files we should search for resource usage. Default is 'm mm swift xid storyboard plist'")
cli.addOption(fileExtOption)

let skipProjReferecCleanOption = BoolOption(longFlag: "skip-proj-reference", helpMessage: "Skip the Project file (.pbxproj) reference cleaning. By skipping it, the project file will be left untouched. You may want to skip ths step if you are trying to build multiple projects with defpendency and keep .pbxproj unchanged whil compiling")
cli.addOption(skipProjReferecCleanOption)
...
do {
    ...
    guard let e = error as? FengNiaoError else {
        print("Unknow Error: \(error)".red.bold)
        exit(EX_USAGE)
    }
    switch e {
...
if !isForce {
    ...
    case .delete:
        break
    case .ignore:
        print("Ignored. Nothing todo， bye!".green.bold)
        exit(EX_OK)
    }
}
...
guard failed.isEmpty else {
    ...
    print("\(unusedFiles.count - failed.count) unused files are deleted. But we encounterd some error while deleting these \(failed.count) files:".yellow.bold)
    for (fileInfo, error) in failed {
        print("\(fileInfo.path.string) - \(error.localizedDescription)")
    }
    exit(EX_USAGE)
}
...
if !skipProjReferecCleanOption.value {
    if let children = try? Path(projectPath).absolute().children() {
        ...
        print("Unused Refeerence deleted successfully.".green.bold)
    }
}
